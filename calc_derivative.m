function [state, Fstate, current] = calc_derivative(time, state, current, Jxfer, Jtr, ICa, Ito2,KSR_factor, NCX_factor,ksat_factor,vmaxr_factor)
...
% max. background Na+  current conductance (mS/uF)
 ...
 vmaxr=1.53*137.0e-6*vmaxr_factor; % Ca++ ATPase backward rate parameter (mM/ms)
...
if(vclamp_flag == 0)
  ...
  Istim = 0.0;
	...
	a3 = 1.0+(ksat*ksat_factor)*exp_etaVFRT/exp_VFRT;
	a4 = KmCa+Cao;
	a5 = 5000.0/(KmNa*KmNa*KmNa+Nao*Nao*Nao);
	INaCa = NCX_factor*kNaCa*a5*(a1-a2)/(a4*a3);
 ...
 %Kv1.4 channel transitions
    ...
    a1 = Acap/(Vmyo(1)*Faraday*1000.0); 
    %dNai = -( INa+INab+3.0*(INaCa+INaK)+IKv14_Na )*a1;
    dNai = 0;
    
    a3 = IKr+IKs+IK1+IKp;
	dKi = -( a3-2.0*INaK+(Ito1-IKv14_Na))*a1;
    
    a3 = ICab -2.0*INaCa + IpCa;
	dCai = beta_i*(Jxfer*(VSS/Vmyo(1)) -Jup -Jtrpn - a3*0.5*a1);
    %dCai =  0;

    dCaNSR = Jup*(Vmyo(1)/VNSR(1)) - Jtr*(VJSR/VNSR(1));
    %dCaNSR = 0;
	dCaTOT =  -(a3+ICa)*0.5*a1*Vmyo(1)*1.0e6;
    ...
    current(index_JDHPR) = ICa*Acap/(-2.0*Vmyo(1)*Faraday*1000.0);
    current(index_Jup) = Jup;
    current(index_Jtrpn) = Jtrpn ;
    current(index_Jtr) = Jtr*VJSR/Vmyo(1);
    current(index_Jxfer) = Jxfer*VSS/Vmyo(1);
