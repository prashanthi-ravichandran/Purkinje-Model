...
filenumber = 1; 
%output_dir = 'Trial';
output_dir = strcat('Freq_',num2str(freq),'Hz_NSRint_',num2str(SR_factor),'X_SERCA_',num2str(KSR_factor),'X_vmaxr_',num2str(vmaxr_factor),'X_k52mod_KeizerLevine_nshells_80_1');
mkdir(output_dir);
...
if(ic_clamp == 1)
    ...
    if(ic_clamp==2)       
        input_dir = strcat('Freq_',num2str(freq),'Hz_NSRint_',num2str(SR_factor),'X_SERCA_',num2str(KSR_factor),'X_vmaxr_',num2str(vmaxr_factor),'X_k52mod_nshells_80');
...
output_RyR_int_file = strcat(output_dir,'/','RyR_int',num2str(filenumber),'.txt');
...
fclose(fileID);
fileID = fopen(output_RyR_int_file,'w');
...
if(ic_clamp ~= 2)
   ...
   [ state, FRU_state,CaCSR_CPU, LType_state, RyR_ss_state,...
           ...
           HTRPNCaArray] = initialize2(ic_states_file, ic_FRU_file,...
                                       ...
                                       ic_HTRPNCa_file, ic_LTRPNCa_file, ic_CaCSR_file);
    if(iscolumn(state))
	state = state';
    end
    LCC1 = LType_state(:,1,1).*onez_SS;
...
%      FRUdep_states(3) = state(8);
     ...
     [state, Fstate, currents]             = calc_derivative(time, state, currents, Jxfer(1), Jtr(1), ICa(1), Ito2(1), KSR_factor, NCX_factor, ksat_factor,vmaxr_factor);
     ...
     Jtr_int = zeros(1, n_shells-n_shells_ss);
     JRyR_int = zeros(1, n_shells-n_shells_ss);
     for s = n_shells_ss+1:n_shells
     ...
     [dCaiArray, dCaNSRArray,dLTRPNCaArray, dHTRPNCaArray, JMyo_diff, JNSR_diff, Jup, Jtrpn] = calc_derivative_int(CaiArray, CaNSRArray, LTRPNCaArray, HTRPNCaArray,subsarc_state,Jtr, Jxfer, Jtr_int, JRyR_int, diff_factor, KSR_factor, NCX_factor);
