...
%%
figure
plot(PRyR_open(5000:10000));
hold on
plot(PRyR_ready(5000:10000));
plot(ones(5001,1) - PRyR_open(5000:10000)-PRyR_ready(5000:10000));
legend('Open','Ready','Adapted')
title('1 Hz')
xlim([0 5000])
%%
figure
...
cai_whole_cell = horzcat(Cai(1:8000,:), fliplr(Cai(1:8000,:)));
...
%%
 fileID = fopen('RyR_int2.txt','r');
formatSpec = '%s';
C_data  = textscan(fileID,'%s %s %s %s %s');
RyR = [];
for i = 1:4
RyR(:,i) = str2double(C_data{i+1}(2:end));
end
RyR(:,5) = RyR(:,1) + RyR(:,2) + RyR(:,3) + RyR(:,4);
figure
plot(RyR(:,1)./RyR(:,5));
hold on
plot((RyR(:,2) + RyR(:,4))./RyR(:,5));
plot(RyR(:,3)./RyR(:,5));
% plot(RyR(:,4)./RyR(:,5));
legend('Ready','Open','Adapted');
xlim([5000 10000])
title('Interior RyR occupancy')
...
xlabel('Time (ms)')
ylabel('JRyR (mM/ms)')
title('Interior RyR flux')

%%
figure
subplot(3,1,1)
title('Open')
plot(PRyR_open_0_5Hz);
hold all
plot(PRyR_open_1Hz);
plot(PRyR_open_2Hz);
xlim([0 5000])
legend('0.5Hz','1Hz','2Hz')

subplot(3,1,2)
title('Ready')
plot(PRyR_ready_0_5Hz);
hold all
plot(PRyR_ready_1Hz);
plot(PRyR_ready_2Hz);
xlim([0 5000])
...
subplot(3,1,3)
title('Adapted')
plot(PRyR_adapted_0_5Hz);
hold all
plot(PRyR_adapted_1Hz);
plot(PRyR_adapted_2Hz);
xlim([0 5000])
legend('0.5Hz','1Hz','2Hz')
...
%% 
sys_1Hz = [Cai_1Hz(34),Cai_1Hz(1034),Cai_1Hz(2033)];
dia_1Hz = [Cai_1Hz(11),Cai_1Hz(1011),Cai_1Hz(2010)];
amp_1Hz = sys_1Hz - dia_1Hz;

sys_2Hz = [Cai_2Hz(35),Cai_2Hz(535),Cai_2Hz(1035)];
dia_2Hz = [Cai_2Hz(11),Cai_2Hz(511),Cai_2Hz(1011)];
amp_2Hz = sys_2Hz - dia_2Hz;

sys_0_5Hz = [Cai_0_5Hz(33),Cai_0_5Hz(2033),Cai_0_5Hz(4033)];
dia_0_5Hz = [Cai_0_5Hz(11),Cai_0_5Hz(2010),Cai_0_5Hz(4011)];
amp_0_5Hz = sys_0_5Hz - dia_0_5Hz;

freq = [0.5,1,2];
sys = [mean(sys_0_5Hz), mean(sys_1Hz), mean(sys_2Hz)];
sys_err = [std(sys_0_5Hz), std(sys_1Hz), std(sys_2Hz)];
dia = [mean(dia_0_5Hz), mean(dia_1Hz), mean(dia_2Hz)];
dia_err = [std(dia_0_5Hz), std(dia_1Hz), std(dia_2Hz)];
amp = [mean(amp_0_5Hz), mean(amp_1Hz), mean(amp_2Hz)];
amp_err = [std(amp_0_5Hz), std(amp_1Hz), std(amp_2Hz)];

figure
errorbar(freq,amp,amp_err);
hold on
errorbar(freq,dia,dia_err);
errorbar(freq,sys,sys_err);
legend('Amplitude','Diastole','Systole')
xlim([0 3])
ylim([0 1.2e-3])
