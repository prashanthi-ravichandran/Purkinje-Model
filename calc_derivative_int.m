function [dCaiArray, dCaNSRArray,dLTRPNCaArray, dHTRPNCaArray,  JMyo_diff, JNSR_diff, Jup, Jtrpn] = calc_derivative_int(CaiArray, CaNSRArray, LTRPNCaArray, HTRPNCaArray,subsarc_state,Jtr, Jxfer, Jtr_int, JRyR_int, diff_factor, KSR_factor, NCX_factor)

global r Vmyo VNSR n_shells dr n_shells_ss VSS VJSR
...
for s = n_shells_ss+1:n_shells
        dCaiArray(s) = beta_i(s)*(-Jup(s) -Jtrpn(s) + JRyR_int(s-n_shells_ss)*(VCSR/Vmyo(s))) + JMyo_diff(s);
        dCaNSRArray(s) = Jup(s)*(Vmyo(s)/VNSR(s))  - Jtr_int(s-n_shells_ss)*(VCSR/VNSR(s)) + JNSR_diff(s);
end
